...
gdf_majeures = gdf_roads_proj[
    gdf_roads_proj['fclass'].isin(ROUTES_MAJEURES)
].copy()

gdf_majeures_exploded = (
    gdf_majeures
...
for i, row in gdf_villes_proj.iterrows():
    ...
    noms_voisins = (
        gdf_villes_proj
        .iloc[indices_voisins]['unique_name']
        .tolist()
    )
    adjacences_voronoi[id_ville] = noms_voisins

print("Extraction des adjacences par routes majeures...")
for idx, route_row in tqdm(
    gdf_majeures_exploded.iterrows(),
    total=len(gdf_majeures_exploded),
    desc="Routes"
):
    route_geom = route_row.geometry
    villes_route = villes_sur_route(
        route_geom, gdf_villes_proj, buffer_m=5000
    )
...
for i, row in tqdm(
    gdf_villes_proj.iterrows(),
    total=len(gdf_villes_proj),
    desc="Fusion"
):
    id_ville = row['unique_name']
    ...
    if len(adjacents) > 30:
        ...
        for adj_ville in adjacents:
            idx_adj = gdf_villes_proj[
                gdf_villes_proj['unique_name'] == adj_ville
            ].index[0]

            dist = point_ville.distance(
                gdf_villes_proj.iloc[idx_adj].geometry
            )
...
print(
    f"Adjacences moyennes par route: "
    f"{sum(len(v) for v in adjacences_routes.values()) / max(1, len(adjacences_routes)):.1f}"
)
print(
    f"Adjacences moyennes finales: "
    f"{sum(len(v) for v in adjacences_finale.values()) / len(adjacences_finale):.1f}"
)

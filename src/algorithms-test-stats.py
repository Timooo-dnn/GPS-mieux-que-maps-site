...
def calculer_itineraire(ville_depart, ville_destination):
    global visited_global, liste, dico, premiere_visite, stats
  ...
  # Reset stats
    ...
    stats["cpu_time"] = round(end_time - start_time, 6)
    
    # Retourner les résultats formatés
    print(stats)
    return formalisation_donnees(dico_3_chemins, distance_entree, temps_entree), stats
...
def parcours_dist_orth(ville, villeA, chemin):
    global premiere_visite, stats
    voisinestri=[]
    stats['sommets_explores']+=1
    ...
    if villeA in maping[ville]:
        ...
        if chemin_final not in liste:
            ...
            stats["relachements"] += 1

        return chemin_final    
    voisines=[]
    for voisine in maping[ville] :
        stats["aretes_examinees"] += 1
...
print(calculer_itineraire("Toulouse_26686518", "Tarbes_26691527"))

print("\n--- STATISTIQUES ---")
print(f"Temps CPU : {stats['cpu_time']} s")
print(f"Sommets explorés : {stats['sommets_explores']}")
print(f"Arêtes examinées : {stats['aretes_examinees']}")
print(f"Relâchements : {stats['relachements']}")
print(f"Opérations file priorité : {stats['ops_file_priorite']}")
print(stats)